...
// ─── Helpers for Real Account Sandbox ────────────────────────────────────────

export const isDemoMode = (): boolean => {
  if (typeof window === 'undefined') return true;
  try {
    const userStr = localStorage.getItem('campusos_user');
    if (!userStr) return true; // Default to showing landing page/mock previews if not logged in
...
export function wrapArray<T>(getMockData: () => T[], fallback: T[] = []): T[] {
  return new Proxy([] as T[], {
    get(target, prop) {
      const activeData = isDemoMode() ? getMockData() : fallback;
      let value = Reflect.get(activeData, prop);
...
const emptyStats: StatCard[] = [
  { id: 's1', label: 'Active Members',    value: 0,  icon: 'Users',        accent: 'navy'    },
  { id: 's2', label: 'Events This Month', value: 0,  icon: 'CalendarDays', accent: 'sand'    },
  { id: 's3', label: 'Projects Live',     value: 0,  icon: 'FolderKanban', accent: 'success' },
  { id: 's4', label: 'Club Points',       value: 0, suffix: 'pts', icon: 'Trophy', accent: 'warning' },
];
export const mockStats = wrapArray(() => rawStats, emptyStats);

const rawTodayEvents: DashboardEvent[] = [
...
export const mockTodayEvents = wrapArray(() => rawTodayEvents);
...
export const mockTimeline = wrapArray(() => rawTimeline);
...
export const mockMeetings = wrapArray(() => rawMeetings);
...
export const mockAnnouncements = wrapArray(() => rawAnnouncements);
...
export const mockNotifications = wrapArray(() => rawNotifications);
...
export function buildMockCalendar(eventsList?: any[]): CalendarDay[] {
  ...
  if (eventsList && eventsList.length > 0) {
    ...
    // If not demo mode and event list is empty, don't show mock calendar highlights
    if (isDemoMode()) {
...
const rawMyRegisteredEvents: DashboardEvent[] = [
...
export const mockMyRegisteredEvents = wrapArray(() => rawMyRegisteredEvents);

const rawMemberStats: StatCard[] = [
...
const emptyMemberStats: StatCard[] = [
  { id: 'ms1', label: 'Events Attended',    value: 0,  icon: 'CalendarDays', accent: 'navy'    },
  { id: 'ms2', label: 'Leaderboard Rank',   value: 0,  icon: 'Trophy',       accent: 'warning' },
  { id: 'ms3', label: 'Club Points',        value: 0, suffix: 'pts', icon: 'Star', accent: 'sand' },
  { id: 'ms4', label: 'Certificates',       value: 0,  icon: 'Award',        accent: 'success' },
];
export const mockMemberStats = wrapArray(() => rawMemberStats, emptyMemberStats);

// ─── Lead-specific mock data ─────────────────────────────────────────────────

const rawLeadStats: StatCard[] = [
...
const emptyLeadStats: StatCard[] = [
  { id: 'ls1', label: 'Team Members',       value: 0,  icon: 'Users',        accent: 'navy'    },
  { id: 'ls2', label: 'Active Projects',    value: 0,  icon: 'FolderKanban', accent: 'success' },
  { id: 'ls3', label: 'Events This Month',  value: 0,  icon: 'CalendarDays', accent: 'sand'    },
  { id: 'ls4', label: 'Pending Approvals',  value: 0,  icon: 'Clock',        accent: 'warning' },
];
export const mockLeadStats = wrapArray(() => rawLeadStats, emptyLeadStats);

const rawClubMembers: ClubMember[] = [
...
export const mockClubMembers = wrapArray(() => rawClubMembers);
...
export const mockPendingApprovals = wrapArray(() => rawPendingApprovals);

const rawRecentRegistrations: { name: string; event: string; time: string; avatarUrl?: string }[] = [
...
export const mockRecentRegistrations = wrapArray(() => rawRecentRegistrations);

// ─── Faculty-specific mock data ───────────────────────────────────────────────

const rawFacultyStats: StatCard[] = [
...
const emptyFacultyStats: StatCard[] = [
  { id: 'fs1', label: 'Active Clubs',       value: 0,   icon: 'Layers',       accent: 'navy'    },
  { id: 'fs2', label: 'Total Members',      value: 0,   icon: 'Users',        accent: 'success' },
  { id: 'fs3', label: 'Pending Approvals',  value: 0,   icon: 'Clock',        accent: 'warning' },
  { id: 'fs4', label: 'Events This Month',  value: 0,   icon: 'CalendarDays', accent: 'sand'    },
];
export const mockFacultyStats = wrapArray(() => rawFacultyStats, emptyFacultyStats);

const rawClubOverviews: ClubOverview[] = [
...
export const mockClubOverviews = wrapArray(() => rawClubOverviews);

const rawReports: ReportItem[] = [
...
export const mockReports = wrapArray(() => rawReports);

const rawFacultyTimeline: TimelineEntry[] = [
...
export const mockFacultyTimeline = wrapArray(() => rawFacultyTimeline);
...
export const mockLeaderboard = wrapArray(() => rawLeaderboard);
...
export const mockBlogPosts = wrapArray(() => rawBlogPosts);
...
export const mockGallery = wrapArray(() => rawGallery);
