      ...
      {filtered.length === 0 ? (
        <FadeIn delay={0.08}>
          <div className="card-surface p-12 text-center max-w-lg mx-auto mt-8 flex flex-col items-center">
            ...
            <h3 className="text-lg font-bold text-ink mb-2">No Events Found</h3>
            <p className="text-sm text-ink-soft mb-6 leading-relaxed">
              There are no events matching your filter, or none have been scheduled yet. Click "Create Event" to plan a new one!
            </p>
            {user?.role === 'lead' && (
              <Button leftIcon="Plus" onClick={() => setIsCreateOpen(true)}>Create Event</Button>
            )}
          </div>
        </FadeIn>
      ) : (
